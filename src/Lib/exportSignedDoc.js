 ...
 *
 * NOTE ON QUALITY: html2canvas can only capture pixels that already exist on
 * screen. If the underlying <Page> (react-pdf) canvas is rendered at a low
 * internal resolution (matched to pageWidth), no amount of scale/format
 * tweaking here will make it sharper — it just upscales blur and bloats
 * file size. The real fix is rendering react-pdf's <Page> at a higher
 * internal resolution (e.g. via a devicePixelRatio/scale prop) in
 * DocumentPreview.jsx. This file only controls capture + PDF packaging.
 */
